...
def get_default_callbacks(config, train_config, version):
    """
    Default callback is just TrainCheckpoint.
    ...
    ckpt_filename = train_config["model_ckpt_filename"].format(version)
    ckpt_path = output_dir / ckpt_filename

    # model checkpointing
    checkpoint_callback = TrainCheckpoint(
        filepath=ckpt_path,  # checkpoint file path
        epsilon=train_config["ckpt_epsilon"],  # minimum improvement to save a new checkpoint
        mode=train_config["callback_metrics_mode"],
    )
